-- =====================================================
-- SISTEMA DE AGENTES ESPECIALIZADOS - PARTE 2
-- =====================================================

-- Adicionar novos valores ao enum product_category se não existirem
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'energia_solar' AND enumtypid = 'product_category'::regtype) THEN
        ALTER TYPE product_category ADD VALUE 'energia_solar';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'telha_shingle' AND enumtypid = 'product_category'::regtype) THEN
        ALTER TYPE product_category ADD VALUE 'telha_shingle';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'steel_frame' AND enumtypid = 'product_category'::regtype) THEN
        ALTER TYPE product_category ADD VALUE 'steel_frame';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'drywall_divisorias' AND enumtypid = 'product_category'::regtype) THEN
        ALTER TYPE product_category ADD VALUE 'drywall_divisorias';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'forros' AND enumtypid = 'product_category'::regtype) THEN
        ALTER TYPE product_category ADD VALUE 'forros';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'saudacao' AND enumtypid = 'product_category'::regtype) THEN
        ALTER TYPE product_category ADD VALUE 'saudacao';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'institucional' AND enumtypid = 'product_category'::regtype) THEN
        ALTER TYPE product_category ADD VALUE 'institucional';
    END IF;
END $$;

-- Criar enum agent_type se não existir
DO $$ 
BEGIN
...
-- Adicionar campos para agente atual na conversa
...
DROP TRIGGER IF EXISTS update_extracted_contexts_updated_at ON extracted_contexts;
...
-- Inserir configurações dos agentes apenas se não existirem
INSERT INTO agent_configs (agent_name, agent_type, product_category, system_prompt, is_spy, temperature, max_tokens) 
SELECT * FROM (VALUES
  ('Atendimento Geral', 'general'::agent_type, NULL::product_category, 'Você é um assistente de atendimento geral da Drystore. Sua função é cumprimentar o cliente cordialmente e descobrir em qual produto ou serviço ele está interessado. 
...
Mantenha as respostas concisas e diretas.', false, 0.7, 200),

  ('Classificador Master', 'classifier'::agent_type, NULL::product_category, 'Você é um agente classificador da Drystore. Analise a conversa e identifique sobre qual categoria de produto o cliente está falando.
...
Retorne APENAS um JSON no formato:
{
  ...
  ('Extrator de Dados', 'extractor'::agent_type, NULL::product_category, 'Você é um agente extrator de dados da Drystore. Identifique e extraia informações relevantes do cliente a partir da conversa.
...
Use null para campos não mencionados.', true, 0.1, 200),

  ('Especialista Energia Solar', 'specialist'::agent_type, 'energia_solar'::product_category, 'Você é especialista em energia solar da Drystore, parceiro oficial da GE. Ajude o cliente com informações técnicas, orçamentos e benefícios da energia solar.
...
Responda de forma técnica mas didática. Faça perguntas relevantes sobre consumo de energia, tipo de telhado, área disponível. Sempre mencione nossa parceria com a GE.', false, 0.7, 300),

  ('Especialista Steel Frame', 'specialist'::agent_type, 'steel_frame'::product_category, 'Você é especialista em Steel Frame da Drystore. Ajude com informações sobre construção em estrutura metálica.
...
Pergunte sobre área da construção, tipo de projeto (residencial/comercial), se tem projeto arquitetônico, prazo desejado. Enfatize velocidade e qualidade.', false, 0.7, 300),

  ('Especialista Telhas Shingle', 'specialist'::agent_type, 'telha_shingle'::product_category, 'Você é especialista em Telhas Shingle da Drystore, representante "Telhado dos Sonhos".
...
Pergunte sobre área do telhado, estilo arquitetônico desejado, se é construção nova ou reforma. Sempre mencione nossa marca "Telhado dos Sonhos".', false, 0.7, 300),

  ('Especialista Ferramentas', 'specialist'::agent_type, 'ferramentas'::product_category, 'Você é especialista em ferramentas profissionais da Drystore.
...
Pergunte sobre tipo de trabalho, frequência de uso (profissional/doméstico), orçamento disponível. Recomende baseado na necessidade real.', false, 0.7, 300)
) AS new_agents(agent_name, agent_type, product_category, system_prompt, is_spy, temperature, max_tokens)
WHERE NOT EXISTS (
  SELECT 1 FROM agent_configs WHERE agent_name = new_agents.agent_name
);
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Enable all for authenticated users' AND tablename = 'agent_configs') THEN
        CREATE POLICY "Enable all for authenticated users" ON agent_configs FOR ALL USING (true);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Enable all for authenticated users' AND tablename = 'classification_history') THEN
        CREATE POLICY "Enable all for authenticated users" ON classification_history FOR ALL USING (true);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Enable all for authenticated users' AND tablename = 'extracted_contexts') THEN
